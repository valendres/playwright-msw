...
describe("worker", () => {
  beforeEach(() => {
    ...
    // Setup spies
    jest.spyOn(Router.prototype, "initialise");
    jest.spyOn(Router.prototype, "use");
    jest.spyOn(Router.prototype, "resetHandlers");
  });

  it("should initialise the router with the provided initialHandlers", async () => {
    const handlers = [rest.get("/api/users", successResolver)];
    const page = mockPage();
    await setupWorker(page, ...handlers);
    expect(Router.prototype.initialise).toBeCalledTimes(1);
    expect(Router.prototype.initialise).toBeCalledWith(...handlers);
  });

  it("should still initialise on the router even if no initialHandlers were provided", async () => {
    const page = mockPage();
    await setupWorker(page);
    expect(Router.prototype.initialise).toBeCalledTimes(1);
    expect(Router.prototype.initialise).toBeCalledWith();
  });
